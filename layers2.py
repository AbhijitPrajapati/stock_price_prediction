...
def create_lstm_layer(num_units, input_shape):
    ...
    def lstm_layer(inputs, params, **kwargs):
        # previous hidden state and previous cell state
        prev_hidden_state = kwargs.get('previous_hidden_state', np.zeros((inputs.shape[0], num_units)))
        prev_cell_state = kwargs.get('previous_cell_state', np.zeros((inputs.shape[0], num_units)))
