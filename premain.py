...
class MultilayerPerceptron:
    ...
    def __call__(self, inputs):
        # put inputs into the input layer
        next_inputs = inputs
    ...
    def backward(self, outputs, actual_outputs):
        ...
        self.layers[-1].neuron_grads = output_n_grad
        ...
        while layer_ind > 0:
            ...
            # output -> matrix with shape (num_previous neurons, num_current_neurons)
            x = self.layers[layer_ind].weights.T * self.layers[layer_ind - 1].activation_derivatives


            # previous layer neuron gradients = x * current layer neuron gradients
            # output -> matrix with shape (num_prev_neruons, 1)
            n_grads = np.dot(x , self.layers[layer_ind].neuron_grads)
...
# mlp.backward(pred, [[-2]])
# quit()
mlp.train(dataset['X'], dataset['Y'], learning_rate=0.01, epochs=50)